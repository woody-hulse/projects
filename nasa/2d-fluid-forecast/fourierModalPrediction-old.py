...
def fourierTransform(
    ...
    :param featureTemporalModes:       temporal modal coefficients of data, shape=(features, snapshots, modes)
    :param trainingLength:      snapshot length of training dataset
    :param predictionLength:    snapshot length of prediction dataset
    ...
    for f in range(numFeatures):
        for m in range(numModes):
            fourierTransformData = np.fft.fft(featureTemporalModes[f][m])
            fourierTransformDataMagnitude = np.abs(fourierTransformData) / trainingLength

            plt.plot(np.fft.ifft(fourierTransformData))
            plt.plot(featureTemporalModes[f][m])
            plt.show()

            plt.plot(fourierTransformDataMagnitude)
            plt.show()

            # construct sine waves
            totalLength = trainingLength + predictionLength
            prediction = np.array([0.0 for _ in range(totalLength)])
            for s in range(trainingLength):
                prediction += np.array([fourierTransformDataMagnitude[s] * np.sin(s*t) for t in range(totalLength)])
...
def main(train=False,
    ...
    with warnings.catch_warnings():
        ...
        featurePredictedTemporalModes = fourierTransform(
            featureTemporalModes,
            trainingLength,
            testingLength,
            numModes,
