...
def createModel(numModes, resolution, numFeatures, lookback, summary=True):
    ...
    LSTMLayer = layers.LSTM(64, return_sequences=True)(MaxPooling1DLayer1)
...
def trainModel(
    ...
    filepath = "differentiatedGroupedModels/" + groupname
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                if trainingLength != -1:
                    ...
                    axis[i, j].plot(range(dataLen - predLen, dataLen), predictedTemporalModes[modeNum], 'y',
                                    label="predicted")
                else:
...
def main(train=False,
    ...
    with warnings.catch_warnings():
        ...
        featurePredictedDifferentiatedTemporalModes = trainModel(
            featureInputTrainingTemporalModes,
            featureOutputTrainingTemporalModes,
            testingLength,
            numModes,
            resolution,
        ...
        if viewModes:
            for f in range(numFeatures):
                plt.cla()
                plt.imshow(featurePredictedDifferentiatedTemporalModes[f][10:].T)
                ax = plt.gca()
                ax.invert_yaxis()
                ax.get_xaxis().set_visible(False)
                ax.get_yaxis().set_visible(False)
    ...
    for f in range(numFeatures):
        ...
        featurePredictedDifferentiatedTemporalModes[f] = scaleData(featurePredictedDifferentiatedTemporalModes[f], featureScalars[f], True)
        featureCombinedTemporalModes.append(reconstructDifferentiation(featureTemporalModes[f], featurePredictedDifferentiatedTemporalModes[f]))
    ...
    plotTemporalModes(featurePredictedDifferentiatedTemporalModes[0], featureCorrectDifferentiatedTemporalModes[0], numModes)

    featurePredictedData = []
    plotTemporalModes(featurePredictedTemporalModes[0], featureCorrectTemporalModes[0], numModes, trainingLength)
